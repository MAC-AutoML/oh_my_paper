...
| LaTeX | 当前本机可用 `latexmk`、`xelatex`、`bibtex` | `paper-ai-latex` / `compile-latex` 的可选 PDF 编译依赖。 |
...
- 🧠 **4 个用户级主技能**与 **15 个 helper 技能**并存，适合不同粒度控制。
...
- 🧾 **内置 LaTeX 导出**：`paper-ai-latex` 内置 arXiv-style 模板，可整理 BibTeX，并在本机 TeX 工具存在时用 XeLaTeX/BibTeX 编译。
- 🖼️ **Demo 已生成真实图像**：`demo/figures/generated/*.jpg` 是基于 figure prompt cards 通过 Nano Banana 2 / `gemini-3.1-flash-image-preview` 生成的图。
...
| 🧾 LaTeX 打包 | `paper-ai-latex` | 复制内置 arXiv-style 模板、映射章节、维护 BibTeX，并在 TeX 工具存在时编译 PDF。 |
...
用内置 arXiv-style 模板编译 LaTeX 工作区：

```bash
# 准备好 sections/references 对应的 LaTeX workspace 后
uv run oh-my-paper compile-latex /tmp/ohmp-paper-run/latex
```

LaTeX skill 内置模板改编自 `MAC-AutoML/Arxiv_Template`；当前内置副本已移除指定的大学 logo 资源和页眉引用。
