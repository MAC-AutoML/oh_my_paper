...
class ProjectConfig:
    ...
    def semantic_api_key(self) -> str | None:
        env_name = str(self.data.get("semantic_scholar", {}).get("api_key_env", "s2_api_key"))
        value = self.env.get(env_name) or self.env.get(env_name.upper())
...
def load_project_config(config_path: str | Path | None = None, *, root: str | Path | None = None) -> ProjectConfig:
    ...
    data = _resolve_env_refs(data, env_values)
    ...
    return ProjectConfig(data=data, sources=sources, config_path=chosen, root=repo, env=env_values)
...
def config_status_report(config_path: str | Path | None = None, *, root: str | Path | None = None, workspace: str | Path | None = None) -> dict[str, Any]:
    ...
    if mode == "api_key" and not config.semantic_api_key():
        warnings.append(f"semantic_scholar.mode api_key requires {semantic.get('api_key_env', 's2_api_key')}")
    report = {
        ...
        "models": _redacted_models(data.get("models", {}), config.env),
        "semantic_scholar": {
            "mode": mode,
            "effective_mode": effective_mode if effective_mode != "auto" else "no_key",
            "api_key_env": semantic.get("api_key_env", "s2_api_key"),
...
def _request_interval(semantic: dict[str, Any], effective_mode: str) -> float:
    ...
    return float(semantic.get(key, 1.1 if effective_mode == "api_key" else 2.0))


def _redacted_models(models: dict[str, Any], env: dict[str, str]) -> dict[str, Any]:
...
def _resolve_env_refs(value: Any, env: dict[str, str]) -> Any:
    if isinstance(value, dict):
        return {key: _resolve_env_refs(item, env) for key, item in value.items()}
    if isinstance(value, list):
        return [_resolve_env_refs(item, env) for item in value]
